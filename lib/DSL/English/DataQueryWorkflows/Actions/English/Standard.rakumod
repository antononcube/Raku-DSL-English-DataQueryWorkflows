...
class DSL::English::DataQueryWorkflows::Actions::English::Standard
	...
	method replace-missing-command($/) {
		...
		make 'replace missing values with ' ~ $na;
	}
	...
	method column-names-command($/) { make 'show column names'; }
	method row-names-command($/) { make 'show row names'; }
	...
	# Pull column command
	method pull-column-command($/) { make 'pull the values of the column ' ~ $<column-name-spec>.made; }
}
