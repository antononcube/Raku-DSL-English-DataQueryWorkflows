...
class DSL::English::DataQueryWorkflows::Actions::Raku::SQL::Builder
    ...
    has Str $.name = 'DSL-English-DataQueryWorkflows-Raku-SQL-Builder';
    ...
    method select-columns-by-pairs($/) {
        make 'select(' ~ $<as-pairs-list>.made ~ ')';
    }

    # Filter commands
    method filter-command($/) {
        make 'where(' ~ $<filter-spec>.made ~ ')';
    }
    method filter-spec($/) {
    ...
    method arrange-by-command-ascending($/) {
        make 'order-by(' ~ $<arrange-by-spec>.made ~ ')';
    }
    method arrange-by-command-descending($/) {
        ...
        make 'order-by(' ~ @cols.map({'desc(' ~  $_  ~ ')' }).join(', ') ~ ')';
    }
    ...
    method full-join-spec($/) {
        if $<join-by-spec> {
            make 'join(:full, ' ~ $<dataset-name>.made ~ ', :on(' ~ $<join-by-spec>.made ~ '))';
        } else {
            make 'join(:full, ' ~ $<dataset-name>.made ~ ')';
        }
    }

    method inner-join-spec($/) {
        if $<join-by-spec> {
            make 'join(:inner, ' ~ $<dataset-name>.made ~ ', :on(' ~ $<join-by-spec>.made ~ '))';
        } else {
            make 'join(:inner, ' ~ $<dataset-name>.made ~ ')';
        }
    }

    method left-join-spec($/) {
        if $<join-by-spec> {
            make 'join(:left, ' ~ $<dataset-name>.made ~ ', :on(' ~ $<join-by-spec>.made ~ '))';
        } else {
            make 'join(:left, ' ~ $<dataset-name>.made ~ ')';
        }
    }

    method right-join-spec($/) {
        if $<join-by-spec> {
            make 'join(:right, ' ~ $<dataset-name>.made ~ ', :on(' ~ $<join-by-spec>.made ~ '))';
        } else {
            make 'join(:right, ' ~ $<dataset-name>.made ~ ')';
        }
    }

    method semi-join-spec($/) {
        if $<join-by-spec> {
            make 'join(:semi, ' ~ $<dataset-name>.made ~ ', :on(' ~ $<join-by-spec>.made ~ '))';
        } else {
            make 'join(:semi, ' ~ $<dataset-name>.made ~ ')';
        }
    }
    ...
    method make-dictionary-command($/) {
        make 'select( ' ~ $<keycol>.made ~ ', ' ~ $<valcol>.made ~ ' )';
    }
