...
class DSL::English::DataQueryWorkflows::Actions::Bulgarian::Standard
	...
	method use-data-table($/) { make 'използвай таблицата: ' ~ $<mixed-quoted-variable-name>.made; }
	...
	method replace-missing-command($/) {
		...
		make 'замести липсващи стойности с ' ~ $na;
	}
	...
	method column-names-command($/) { make 'покажи имената на колоните'; }
	method row-names-command($/) { make 'покажи имената на редовете'; }
	...
	method semi-join-spec($/)  {
		if $<join-by-spec> {
			...
			make 'анти-съединение с ' ~ $<dataset-name>.made;
		}
	}
	...
	# Pull column command
	method pull-column-command($/) { make 'издърпай стойностите на колоната ' ~ $<column-name-spec>.made; }
}
